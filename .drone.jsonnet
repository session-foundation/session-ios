...
local custom_clone = {
  ...
  commands: [
    ...
    'chmod 600 ~/.ssh/config && chmod 600 ~/.ssh/known_hosts && chmod 600 ~/.ssh/id_ed25519_drone_ci_deploy',
    'eval "$(ssh-agent -s)"',
    'ssh-add ~/.ssh/id_ed25519_drone_ci_deploy',
    'ssh-keyscan -v -t ed25519 github.com >> ~/.ssh/known_hosts',
    'ssh -v -T git@github.com -i ~/.ssh/id_ed25519_drone_ci_deploy -F ~/.ssh/config -o UserKnownHostsFile=~/.ssh/known_hosts',
    'git init',
