...
///
/// A flag accepted by `updateExpiry` and not forwarded to the request passes any test of the caller's arguments and any test
/// of the request type in isolation, so the assertion is made against the encoded body.
class UpdateExpiryRequestSpec: AsyncSpec {
    override class func spec() {
        ...
        func sentBody(shortenOnly: Bool? = nil, extendOnly: Bool? = nil) async -> String? {
            ...
            ///
            /// `wasCalled(atLeast: 1)` above already fails if nothing was sent. These add what it cannot: that the captured
            /// call is this request rather than merely some request, which the "sends neither flag" case needs since it is
            /// made entirely of `toNot(contain:)` assertions
