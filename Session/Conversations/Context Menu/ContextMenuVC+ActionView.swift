...
extension ContextMenuVC {
    final class ActionView: UIView {
        ...
        private func setUpSubtitle() {
            ...
            let timeToExpireInSeconds = max(0, (expiresStartedAtMs + expiresInSeconds * 1000 - dependencies[cache: .storageServer].currentOffsetTimestampMs()) / 1000)
            ...
            timer = Timer.scheduledTimerOnMainThread(withTimeInterval: 1, repeats: true, using: dependencies, block: { [weak self, dependencies] _ in
                let timeToExpireInSeconds: TimeInterval =  (expiresStartedAtMs + expiresInSeconds * 1000 - dependencies[cache: .storageServer].currentOffsetTimestampMs()) / 1000
